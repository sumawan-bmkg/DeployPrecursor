"""
Geomagnetic Data Fetcher - Enhanced with Dual Format Support
Mengambil data dari server geomagnetik via SSH dengan dukungan untuk:
1. Format .STN (uncompressed binary) - 1.47MB per file
2. Format .gz (compressed binary) - 0.40MB per file (3.6x smaller)

Automatically detects and uses the best available format for maximum data availability.
"""
import os
import sys
import paramiko
...
# Local fallback data roots (BMKG mdata dumps, 2018-2023 archives)
LOCAL_DATA_ROOTS = [
    Path('D:/multi/missing/mdata2'),
    Path('D:/multi/missing'),
]


def find_local_file(date, station):
    """
    Search for a local .gz or uncompressed binary file for a given date/station.
    Searches across all LOCAL_DATA_ROOTS.
    ...
    for root in LOCAL_DATA_ROOTS:
        ...
        gz_path = station_dir / f"{filename_base}.gz"
        if gz_path.exists():
            return gz_path, 'gz'
        raw_path = station_dir / f"{filename_base}"
        if raw_path.exists():
            return raw_path, 'stn'
    return None, None


def fetch_local_data(date, station='SCN'):
    """
    Fetch geomagnetic data from local archive files.
    Uses read_604rcsv_new_python (MATLAB-compatible binary parser).

    Args:
        date: datetime object or string 'YYYY-MM-DD'
        station: Station code (default: 'SCN')
        
    Returns:
        dict with Hcomp/Dcomp/Zcomp/etc arrays or None
    """
    if isinstance(date, str):
        date = datetime.strptime(date, '%Y-%m-%d')
    
    # Find which root contains this station's directory
    root_path = None
    for root in LOCAL_DATA_ROOTS:
        stn_dir = root / station
        if stn_dir.exists():
            root_path = root
            break
    
    if root_path is None:
        logger.debug(f"No local root found for station {station}")
        return None
    
    # Use read_604rcsv_new_python which constructs: {path}/{stn}/S{yy}{mm}{dd}.{stn}[.gz]
    # So we pass root_path (e.g. D:/multi/missing/mdata2) and it will find
    # D:/multi/missing/mdata2/{station}/S....{station}.gz
    try:
        result = read_604rcsv_new_python(
            date.year, date.month, date.day, station, str(root_path)
        )
        # read_mdata returns keys: H, D, Z, X, Y, IX, IY, TempS, TempP, Voltage, Time
        # We map them to the same dict format as fetch_data
        num_seconds_in_day = 86400
        H = result['H']
        D = result['D']
        Z = result['Z']
        
        logger.info(f"[LOCAL] {station} {date.date()}: "
                     f"H_mean={np.nanmean(H):.1f}, "
                     f"samples={np.sum(~np.isnan(H))}/{num_seconds_in_day}")
        
        data = {
            'Hcomp': H if len(H) == num_seconds_in_day else np.pad(H, (0, num_seconds_in_day - len(H)), constant_values=np.nan),
            'Dcomp': D if len(D) == num_seconds_in_day else np.pad(D, (0, num_seconds_in_day - len(D)), constant_values=np.nan),
            'Zcomp': Z if len(Z) == num_seconds_in_day else np.pad(Z, (0, num_seconds_in_day - len(Z)), constant_values=np.nan),
            'Xcomp': result.get('X', np.full(num_seconds_in_day, np.nan)),
            'Ycomp': result.get('Y', np.full(num_seconds_in_day, np.nan)),
            'IXcomp': result.get('IX', np.full(num_seconds_in_day, np.nan)),
            'IYcomp': result.get('IY', np.full(num_seconds_in_day, np.nan)),
            'TempS': result.get('TempS', np.full(num_seconds_in_day, np.nan)),
            'TempP': result.get('TempP', np.full(num_seconds_in_day, np.nan)),
            'Voltage': result.get('Voltage', np.full(num_seconds_in_day, np.nan)),
            'Time': np.arange(num_seconds_in_day),
            'date': datetime(date.year, date.month, date.day),
            'station': station,
            'file_info': {
                'path': str(root_path / station),
                'source': 'local',
            },
            'stats': {
                'valid_samples': int(np.sum(~np.isnan(H))),
                'coverage': float(np.sum(~np.isnan(H)) / num_seconds_in_day * 100),
                'h_mean': float(np.nanmean(H)),
                'h_std': float(np.nanstd(H)),
        ...
        logger.warning(f"[LOCAL] Error reading {station} {date.date()}: {e}")
        return None
...
class GeomagneticDataFetcher:
    ...
    def __init__(self, ssh_config=None, prefer_compressed=True, use_local=False):
        """
        ...
        Args:
            ...
            use_local: If True, try local archive first before remote
        """
    ...
    def get_file_paths(self, date, station):
        ...
        if self.prefer_compressed:
            # Prefer compressed files first (smaller download, 3.6x faster)
            paths.extend([
    ...
    def fetch_data(self, date, station='GTO'):
        ...
        Automatically tries both .STN and .gz formats for maximum data availability.
        ...
        # Local-first mode: try local archive first
        if self.use_local:
            logger.info(f"[LOCAL MODE] Trying local archive for {date.date()} {station}")
            local_data = fetch_local_data(date, station)
            if local_data is not None:
                logger.info(f"[LOCAL SUCCESS] {date.date()} {station}")
                return local_data
            else:
                logger.info(f"[LOCAL MISS] Falling back to remote for {date.date()} {station}")

        # Get possible file paths (both .STN and .gz formats)
        ...
        for file_path, format_type, description in possible_paths:
            ...
            try:
                ...
                logger.info(f"[OK] File found! Size: {file_size:,} bytes")
                ...
                with BytesIO() as file_buffer:
                    ...
                    if format_type == "gz":
                        ...
                        logger.info(f"[GZ] Decompressed: {len(compressed_data):,} -> {len(binary_data):,} bytes")
                    else:
                        # Read uncompressed data
                        binary_data = file_buffer.read()
                        logger.info(f"[RAW] Read uncompressed: {len(binary_data):,} bytes")
                    ...
                    data['file_info'] = {
                        ...
                        'compression_ratio': len(binary_data) / file_size if format_type == "gz" else 1.0
                    }
                    
                    logger.info(f"[SUCCESS] Successfully fetched data for {date.date()} using {description}")
                    return data
                    
            except FileNotFoundError:
                logger.info(f"[NOT_FOUND] File not found: {file_path}")
                continue
            except Exception as e:
                logger.error(f"[ERROR] Error reading {file_path}: {e}")
                continue
        
        # If we get here, no files were found in either format
        logger.error(f"[FAILED] No data files found for {date.date()} station {station} (tried both .STN and .gz formats)")
        return None
    ...
    def parse_binary_data(self, binary_data, year, month, day, station):
        ...
        Works with both .STN and .gz formats (already decompressed if needed)
        ...
        Args:
            binary_data: Raw binary data (already decompressed if from .gz)
...
if __name__ == '__main__':
    ...
    logger.info("Testing local archive mode...")
    local_fetcher = GeomagneticDataFetcher(use_local=True)
    test_date = '2023-01-01'
    data = local_fetcher.fetch_data(test_date, 'SCN')
    if data:
        print(f"[LOCAL OK] {data['date']} {data.get('station', 'SCN')}")
        print(f"  Coverage: {data['stats']['coverage']:.1f}%")
        print(f"  H mean: {data['stats']['h_mean']:.2f} nT")
        print(f"  Source: {data.get('file_info', {}).get('source', 'unknown')}")
    else:
        print(f"[LOCAL MISS] No data for {test_date} SCN")
